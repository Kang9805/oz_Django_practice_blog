...
class Blog(TimestampModel):
    ...
    category = models.CharField('카테고리', max_length=30, choices=CATEGORY_CHOICES, default='free')
    ...
    # 2025/9/22일
    # blog/2025/9/22/이미지파일.jpg
    # ImageFields는  FileField와 같지만 이미지만 업로드하게 되어있다.
    # varchar => 실제로는 경로만 저장을 함

    # models.CASCADE -> 같이 삭제
    # models.PROTECT -> 삭제가 불가능함 (유저를 삭제하려고할때 블로그가 있으면 유저 삭제 불가능)
    # models.SET_NULL -> 널 값을 넣는다 ( 유저 삭제시 블로그의 author가 null이 됨)
    ...
    def save(self, *args, **kwargs):
        ...
        thumbnail_name = image_path.stem # /blog/2025/09/22/database.png => database
        thumbnail_extension = image_path.suffix.lower() # /blog/2025/09/22/database.png => .png
